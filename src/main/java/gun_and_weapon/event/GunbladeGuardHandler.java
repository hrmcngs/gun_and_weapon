...
public class GunbladeGuardHandler {
	...
	public static void onPlayerHurt(LivingHurtEvent event) {
		...
		int blockingDuration = useItem.getUseDuration() - useTime;

		if (blockingDuration <= 5) {
			// PERFECT PARRY: within first 5 ticks
			event.setAmount(0);
			event.setCanceled(true);

			DamageSource source = event.getSource();
			if (source.getEntity() instanceof LivingEntity attacker) {
				attacker.hurt(DamageSource.playerAttack(player), 4.0f);
				attacker.knockback(1.0f,
			...
			player.level.playSound(null, player.blockPosition(),
					SoundEvents.SHIELD_BLOCK, SoundSource.PLAYERS, 1.0f, 2.0f);
		} else {
			// NORMAL GUARD: 60% damage reduction
